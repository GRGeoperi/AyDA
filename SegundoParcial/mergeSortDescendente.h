...
void MergeD(int *Array, int Begin, int Middle, int Size)
{
    int i;
    int j;
    int k;
...
void MergeSortD(int *Array, int Begin, int Size)
{
    if (Begin < Size)
    {
        int middle = Begin + (Size - Begin) / 2;
        MergeSortD(Array, Begin, middle);
        MergeSortD(Array, middle + 1, Size);
        MergeD(Array, Begin, middle, Size);
    }
}
